...
export default class Page {
   ...
   * @param draftId 草稿id
   */
  static async getAsyncPageDetailByRoute(
    path: string,
    draftId: string | number
  ) {
    const cacheKey = path + draftId
  ...
  static async getAsyncPageDetail() {
    ...
    const { draftId } = query
    // 然后调接口获取
    const res = await this.getAsyncPageDetailByRoute(
      pathname,
      draftId as string
    )

    const content = _.get(res, 'data.draft.content')
    const projectId = _.get(res, 'data.page.project_id')
